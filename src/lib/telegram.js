...
const MANAGER_USERNAMES = (
  ...
  'sayesstephanie,sayyes_kristina,sayyes_roman,sayesviktoria,dp_1988,olia_pi'
)
  .split(',')
...
export function formatReminder(booking, hoursLeft, options = {}) {
  ...
  return `Напоминание: ваш пробный урок ${timeLabel}!\n\n` +
    ...
    `🔗 <a href="${ZOOM_JOIN_URL}">Подключиться к уроку</a>\n` +
    `Идентификатор ${ZOOM_MEETING_ID}, код доступа ${ZOOM_PASSCODE}\n\n` +
    (hoursLeft <= 2 && !options.skipBonusLine ? `После урока преподаватель соберёт для вас подборку материалов под вашу цель.\n\n` : '') +
...
// Содержательная часть памятки — без ссылки на урок и без прощания.
// Вынесена отдельно, чтобы её можно было приклеить к напоминанию за час одним
// сообщением: тем, кто записался впритык, два сообщения подряд не нужны, а сама
// памятка нужна — она единственное, что объясняет ценность урока и обещает бонус.
export function handoutBody() {
  return `Во время пробного занятия преподаватель:\n` +
