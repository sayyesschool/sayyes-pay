...
export async function GET(request) {
  ...
  try {
    ...
    for (const booking of bookings) {
      ...
      // 8 часов до урока (с 24.09 порог 12 часов, см. ниже), снимать нельзя: просьбу подтвердить шлют за сутки,
      ...
      // 24.09.2026 порог поднят с 8 до 12 часов. Лариса из Нью-Йорка записалась
      // в полночь по своему времени на урок через 9 часов, проспала просьбу
      // подтвердить, и запись сняли утром. 12 часов покрывают ночь в любом поясе.
