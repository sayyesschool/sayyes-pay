...
export async function POST(request) {
  try {
    ...
    //
    // Исключение — урок, который состоялся. 22.09.2026 автоматика сняла запись
    // за неподтверждённость, ученица всё равно пришла, ведущая отметила «Пришёл» —
    // и школа не смогла взять с неё деньги: статус остался «отменена».
    // Если урок отмечен состоявшимся, оплату принимаем: отмена к нему уже
    // отношения не имеет.
    if (booking && booking.status === 'cancelled' && booking.attended !== true) {
      return NextResponse.json({
